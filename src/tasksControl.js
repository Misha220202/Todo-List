  ...
  ? JSON.parse(tasksArrJson).map(
      (taskObj) =>
        new Task(
          taskObj.title,
          taskObj.description,
          taskObj.dueDateFormatted,
          taskObj.checkStatus,
          Number(taskObj.recurringCycle),
          taskObj.importance
        )
    )
  : [];
...
export const initiateTaskArr = () => {
  if (!tasksArrJson) {
    const initialTask1 = new Task(
      'Welcome to the "ToDo-List"',
      'Organize your work and life, finally.',
      currentDateFormatted(),
      'notChecked'
    );
    const initialTask2 = new Task(
      'Create your first task',
      'Clicking "add task" to start.',
      currentDateFormatted(),
      'notChecked',
      0,
      'important'
...
class TaskListNodeManager {
  ...
  groups() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const tasks = tasksArr.filter(
        (task) =>
          task.dueDateFormatted == formattedDate &&
          task.checkStatus == 'notChecked'
      );
      groups.push({ groupName, tasks, date: formattedDate });
    }

    const futureTasks = tasksArr.filter(
      (task) => task.dueDate > this[`day7`] && task.checkStatus == 'notChecked'
    );
    groups.push({
      groupName: 'Future',
      tasks: futureTasks,
      date: `After ${this[`day${7}Formatted`]}`,
    });

    const importantTasks = tasksArr.filter(
      (task) =>
        task.importance == 'important' &&
        task.dueDate >= this[`day1`] &&
        task.checkStatus == 'notChecked'
    );
    groups.push({ groupName: 'Important', tasks: importantTasks, date: `` });

    const overdueTasks = tasksArr.filter(
      (task) =>
        task.dueDate < this[`day1`] &&
        task.checkStatus == 'notChecked' &&
        task.recurringCycle == 0
    );
    groups.push({ groupName: 'OverDue', tasks: overdueTasks, date: `` });

    const completedTasks = tasksArr.filter(
      (task) => task.checkStatus == 'checked'
    );
    ...
    groups.forEach((group) => {
      ...
      if (group.groupName == 'OverDue') {
        ...
        button.addEventListener('click', () => {
          group.tasks.forEach(
            (task) => (task.dueDateFormatted = this[`day1Formatted`])
          );
          this.update();
        });
      }
  ...
  showDomNodes() {
    ...
    const activePanel = Array.from(tasksControlPanel.children).find((panel) =>
      panel.classList.contains('chosen')
    );
  ...
  edit(taskNode, index) {
    ...
    submit.addEventListener('click', () => {
      ...
      tasksArr[index].dueDateFormatted =
        dueDateNodeBeingEdited.firstChild.value;

      tasksArr[index].recurringCycle = parseInt(
  ...
  toggleCheckStatus(index) {
    ...
    toggledTask.checkStatus =
      toggledTask.checkStatus === 'checked' ? 'notChecked' : 'checked';
    this.update();
  }

  toggleImportance(index) {
    const toggledTask = tasksArr[index];
    toggledTask.importance =
      toggledTask.importance === 'important' ? 'notImportant' : 'important';
    this.update();
  }
}
