...
export class Project {
    ...
    get groupNames() {
        const taskGroupNames = this.projectTasksArr.map(projectTask => projectTask.groupName);
        const allGroupNames = [...taskGroupNames];
        if (this.defaultGroupNames) {
            this.defaultGroupNames.forEach(groupName => allGroupNames.splice(0, 0, groupName));
        }
