...
class ProjectsNodeManager {
    ...
    createProjectTaskNode(projectTask) {
        ...
        projectTaskNode.classList.add('task', `${projectTask.checkStatus}`, `${projectTask.importance}`);
    ...
    activateProjectTaskNodeButtonFunctions(projectTaskNode) {
        const iconButtonContainer = projectTaskNode.querySelector('.iconButtonContainer');
    ...
    activateAddTaskButtonFunction() {
        ...
        addTask.addEventListener('click', () => {
            ...
            const optionsArr = [];
            this.project.groupNames.forEach(groupName => optionsArr.push(groupName));
            ...
            submit.addEventListener('click', event => {
                let allValid = true;
                inputs.forEach(input => {
                    if (!input.value && (input.id !== 'CustomGroup' || input.id == 'CustomGroup' && input.parentNode.style.display == 'flex')) {
                ...
                if (allValid == true) {
                    ...
                    const groupNameInput = selectNode.value == 'New Group' ? customOptionInputNode.value : selectNode.value;

                    const projectTask = new ProjectTask(titleInput, descriptionInput, dueDateFormattedInput, 'notChecked', Number(recurringInput), importanceInput, groupNameInput);
    ...
    update() {
        this.project.projectTasksArr.forEach(projectTask => projectTask.checkAndUpdate());
        localStorage.setItem('projectsArr', JSON.stringify(projectsArr));
    ...
    edit(projectTaskNode, index) {
        ...
        submit.addEventListener('click', () => {
            ...
            projectTasksArr[index].dueDateFormatted = dueDateNodeBeingEdited.firstChild.value;

            projectTasksArr[index].recurringCycle = parseInt(recurringCycleBeingEdited.children[1].value);
    ...
    toggleCheckStatus(index) {
        ...
        toggledProjectTask.checkStatus = toggledProjectTask.checkStatus == 'checked' ? 'notChecked' : 'checked';
        this.update();
    }

    toggleImportance(index) {
        const toggledProjectTask = this.project.projectTasksArr[index];
        toggledProjectTask.importance = toggledProjectTask.importance == 'important' ? 'notImportant' : 'important';
        this.update();
    }
}
...
export const projectsControl = () => {
    const createProjectDivNode = project => {
        ...
        projectDiv.querySelector('p').textContent = project.title ? project.title : `My project ${projectsArr.length}`;
        return projectDiv;
    }
    ...
    const activateProjectDivButtonFunctions = projectDiv => {
        ...
        const getIndex = () => projectsArr.findIndex(project => project.id == projectDiv.id);
        let index = getIndex();
        ...
        chevronButton.addEventListener('click', () => {
            editButton.style.display = editButton.style.display == 'block' ? 'none' : 'block';
    ...
    const updateProjectsControlPanel = () => {
        projectsArr.forEach(project =>
            project.projectTasksArr.forEach(projectTask => projectTask.checkAndUpdate()));
        const projectSidebarTitle = projectsControlPanel.querySelector('.sidebarTitle');
